...
namespace SeesharpTools.JXI.Mathematics.example
{
    ...
    ///            Generate a set of random numbers and calculate their standard deviation
    /// Completion date: September 17, 2017
    /// Version: 1.0
    /// Author: Liu Yuhui , JXI    

    public partial class StandardDeviationForm : Form
    {
        ...
        {
            InitializeComponent();
            StandardDeviationEvent(null, null);
        ...
        /// <param name="e"></param>
        private void StandardDeviationEvent(object sender, EventArgs e)
        {
            data = new double[(int)nudLength.Value];
            RandomSequence(ref data, 0, 1);
            easyChart1.Plot(data);
            txtboxStandardDeviation.Text = ProbabilityStatistics.StandardDeviation(data).ToString();
            EfficiencyEvent(null, null);
        }
        ...
        private void EfficiencyEvent(object sender, EventArgs e)
        {
            ...
            {
                ProbabilityStatistics.StandardDeviation(data);
            }
            stopwatch.Stop();
