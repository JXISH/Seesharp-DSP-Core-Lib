...
namespace SeeSharpTools.JXI.SignalProcessing.GeneralSpectrum
{
    internal static class BasicFFT
    {
        ...
        public static void RealFFT(double[] xIn, ref Complex[] xOut)
        {
            ...
            {
                throw new JXIUserBufferException();
            }
        ...
        /// </summary>
        /// <param name="xIn">time domain complex datas, x0.re x0.im x1.re x1.im...</param>
        /// <param name="xOut">complex frequency datas, _outputWaveform[0].re _outputWaveform[0].im _outputWaveform[1].re _outputWaveform[1].im...</param>
        ...
        private static void RealFFT(double[] xIn, ref IntPtr xOut)
        {
            ...
            {
                throw new JXIInnerException("Dfti Compute Forward failed! error code=" + status);
            }
        }
        ...
        {
            int status;
            var fftDesc = DftiDescMgr.GetDFTDesc(xOut.Length, DFTType.Double1DRealInComplexOut);//创建描述符
            //ComputeForward
