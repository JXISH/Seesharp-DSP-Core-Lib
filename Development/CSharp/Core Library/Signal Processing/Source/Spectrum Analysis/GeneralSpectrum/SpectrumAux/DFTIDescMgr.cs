...
namespace SeeSharpTools.JXI.SignalProcessing.GeneralSpectrum
{
    internal class DFTIDescMgr:IDisposable
    {
        ...
        /// <summary>
        /// Get DFTKey 完成创建描述符 配置参数 提交描述符这三个步骤
        /// </summary>
        ...
        public IntPtr GetDFTDesc(int fftSampleCount, DFTType type)
        {
            ...
            lock (_dftDescDict)
            {
                ...
                switch (type)
                {
                    case DFTType.Double1DRealInComplexOut:
                        ...
                        if (0 != (ret = DFTI.DftiCreateDescriptor(ref desc, precision, forwardDomain,
                                                        dimension, length)))
                        {
                            throw new JXIInnerException("DftiCreateDescriptor Failed, error code = " + ret);
                        }

                        if (0 != (ret = DFTI.DftiSetValue(desc, DFTI.PACKED_FORMAT, DFTI.CCS_FORMAT)))
                        {
                            throw new JXIInnerException("DftiSetValue PACKED_FORMAT Failed, error code = " + ret);
                        }
                        break;
                ...
                {
                    throw new JXIInnerException("DftiSetValue Failed, error code = " + ret);
                }

                //配置DFTI_PLACEMENT为DFTI_NOT_INPLACE,即FFT后的结果不覆盖原输入信号
                if (0 != (ret = DFTI.DftiSetValue(desc, DFTI.PLACEMENT, DFTI.NOT_INPLACE)))
                {
                    throw new JXIInnerException("DftiSetValue PLACEMENT Failed, error code = " + ret);
                }

                if (0 != (ret = DFTI.DftiCommitDescriptor(desc)))
                {
                    throw new JXIInnerException("DftiCommitDescriptor Failed, error code = " + ret);
                }
                #endregion
        ...
        private void AddDFTDesc(uint fftSampleCount, DFTType type, IntPtr desc)
        {
            ...
            {
                throw new JXIInnerException("Descriptor have been exist.");
            }
        }
        ...
        public void Dispose()
        {
            ...
            foreach (var desc in _dftDescDict)
            {
                ...
                {
                    throw new JXIInnerException("DftiFreeDescriptor Failed, error code = " + ret);
                }
            }
            _dftDescDict.Clear();
        }
